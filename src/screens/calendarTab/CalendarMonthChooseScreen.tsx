...
type CalendarMonthChooseNavigation = NativeStackNavigationProp<
  CalendarTabStackParamList,
  'CalendarMonthChooseScreen'
>;

type CalendarMonthChooseRoute = NativeStackScreenProps<
  CalendarTabStackParamList,
  'CalendarMonthChooseScreen'
>['route'];
      ...
      <ScrollView
        ...
        {monthLabels.map((monthLabel, monthIndex) => (
          <MonthCard
            ...
            year={year}
            events={
              eventsByMonth[buildEventKey(year, monthIndex)] ?? []
            }
...
export const CalendarMonthChooseScreen = () => {
  const {t} = useTranslation('calendar', {i18n});
  const navigation = useNavigation<CalendarMonthChooseNavigation>();
  const route = useRoute<CalendarMonthChooseRoute>();
  ...
  useEffect(() => {
    prefetchYear(selectedYear);
  }, [prefetchYear, selectedYear]);

  const currentYear = useMemo(() => new Date().getFullYear(), []);
  const {data: calendarData, isError: isCalendarError} = useMergedCalendarYears(
